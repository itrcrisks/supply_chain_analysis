...
def plot_global_basemap(ax,include_countries=None,
    ...
    boundary_gdp = gpd.read_file(os.path.join(
                    ...
                    'ne_10m_admin_0_sovereignty.shp'),encoding="utf-8")
    if include_countries is not None:
        boundary_gdp = boundary_gdp[boundary_gdp["ISO_A3_EH"].isin(include_countries)]
    ...
    if include_countries is not None:
        ...
        xmin = bounds[0]
        xmax = bounds[2]
    ax = get_axes(ax,extent = (xmin,xmax,bounds[1],bounds[3]),epsg=4326) # extent requires (xmin,xmax,ymin,ymax) you might have to adjust the offsets a bit manually as I have done here by +/-0.1
    ...
    if include_labels is True:
        # labels = pd.read_csv(os.path.join(data_path,'region_names.csv'))
        labels = boundary_gdp[['ISO_A3_EH','geometry']]
        labels = labels[labels["ISO_A3_EH"].isin(label_countries)]
...
def point_map_plotting_colors_width(ax,df,column,
    ...
    if plot_title:
        plt.title(plot_title, fontsize=9)
    first_legend = ax.legend(handles=legend_handles,fontsize=legend_size,title=legend_label,loc='lower right')
